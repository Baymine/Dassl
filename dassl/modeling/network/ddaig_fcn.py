...
class LocNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.tanh(x)
        x = x.view(-1, 2, 2)
        theta = x.data.new_zeros(x.size(0), 2, 3)  # 得到指定维度且数据类型于x相同的全零张量
        theta[:, :, :2] = x
        return theta
...
class FCN(nn.Module):
    ...
    def __init__(
        ...
        gctx=True,  # global context fusion layer
        stn=False,
        image_size=32,
    ):
        super().__init__()
    ...
    def forward(self, x, lmda=1.0, return_p=False, return_stn_output=False):
        ...
        if self.gctx_fusion is not None:
            # 自适应池化， 可以将特征图转换成指定的维度，以适应全连接层
            c = F.adaptive_avg_pool2d(x, (1, 1))  # 将特征图缩放至维度为(1,1) (input, output_size)
            c = c.expand_as(x)    # 将c拓展成与x一样的维度，这是为了方便后续与x合并，拓展方式就是简单地复制原有的数据
            x = torch.cat([x, c], 1)
