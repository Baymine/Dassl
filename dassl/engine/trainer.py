...
class SimpleNet(nn.Module):
    ...
    # @property：由SimpleNet.fdim得到模型最终输出的维度
    @property
    def fdim(self):
        return self._fdim

    def forward(self, x, return_feature=False):
        f = self.backbone(x)   # 特征提取器
        if self.head is not None:  # 例如全连接层
            f = self.head(f)

        if self.classifier is None:
            return f

        y = self.classifier(f)  # 分类器，转换成数据类别个数

        if return_feature:  # 返回预测标签以及提取出来的特征
            return y, f
...
# For domain adaption or semi-supervised learning
class TrainerXU(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        train_loader_x_iter = iter(self.train_loader_x)  # iter(): 转换成可迭代对象
        ...
        for self.batch_idx in range(self.num_batches):
            ...
            data_time.update(time.time() - end)  # 添加新的数据（更新其中的统计量）
            ...
            if (
                ...
                nb_remain = 0
                nb_remain += self.num_batches - self.batch_idx - 1  # 剩余的batch数量
                nb_remain += (
            ...
            for name, meter in losses.meters.items():  # 将一些统计量信息写入到tensorboard中
